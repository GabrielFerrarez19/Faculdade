...
function renderCategorias(items = []) {
  ...
  for (const c of items) {
    ...
    div.innerHTML = `
      ...
      <div class="actions">
        <button class="btn-danger" onclick="deleteCategoria(${
          c.id
        })">Excluir</button>
      </div>
    `;
  ...
  for (const p of items) {
    ...
    div.innerHTML = `
      <div>
        ...
        <div class="badge">#${p.id} • ${money(p.preco)} • cat:${
      p.categoria_id
    }</div>
      </div>
      <div class="actions">
        <button class="btn-danger" onclick="deleteProduto(${
          p.id
        })">Excluir</button>
      </div>
    `;
...
async function createProduto({ nome, preco, categoria_id }) {
  await http("POST", `${BASE_URL}/produto`, {
    nome,
    preco: Number(preco),
